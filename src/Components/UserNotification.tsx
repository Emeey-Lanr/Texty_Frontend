...
const UserNotification = () => {
  ...
  const followViaNotificationBtn = (userYouWantToFollow: string) => {
    ...
    followFunction("followSocket2", userDetail.registerdUserIdentification, userYouWantToFollow, "followed you back")
  }
  const unfollowViaNotification = (userYouWantToUnFollow:string) => {
   unfollowFunction("unfollowSocket2",userDetail.registerdUserIdentification, userYouWantToUnFollow) 
  }
  return (
    <>
              ...
              {data.followed && <div className="notification_follow_action">
                <div>
                  {userDetail.following.find((name: { username: string }) => name.username === data.username) ?
                    <button onClick={()=>unfollowViaNotification(data.username)} style={{background:"black",color:"white", border:"none", padding:"10px 20px", borderRadius:"30px"} }>Following</button> :
                    <button onClick={()=>followViaNotificationBtn(data.username)}  style={{background:"none", padding:"10px 20px", border:"1px solid black", borderRadius:"30px" }}>Follow</button>}
                </div>
              </div>}
          </div>
          ))}
